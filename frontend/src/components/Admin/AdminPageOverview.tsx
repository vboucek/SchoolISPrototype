    ...
    <div className="text-black phone:mt-0 laptop:mt-20 phone:mx-2 laptop:mx-20">
            ...
            return (
              ...
              border-black laptop:w-40 phone:w-32 p-2 shadow-md shadow-black">
                ...
                <div className="flex flex-row gap-2">
                  <Link to={`/admin/faculty/${faculty.id}/edit`} className="flex flex-row mt-5 w-32">
                    <button className="bg-gray-400 w-full border-black text-2xl">Edit</button>
                  </Link>
                  <Link to={`/admin/faculty/${faculty.id}/delete`} className="flex flex-row mt-5 w-32">
                    <button className="bg-red-400 w-full border-black text-2xl">Delete</button>
                  </Link>
                </div>
              </div>);
          })}
          ...
          {semesters.map(semester => {
            return (
              <div className="block border-2 border-black laptop:w-32 p-2 shadow-md shadow-black">
                ...
                <div className="flex flex-row gap-2">
                  <Link to={`/admin/semester/${semester.id}/edit`} className="flex flex-row mt-5 w-32">
                    <button className="bg-gray-400 w-full border-black text-2xl">Edit</button>
                  </Link>
                  <Link to={`/admin/semester/${semester.id}/delete`} className="flex flex-row mt-5 w-32">
                    <button className="bg-red-400 w-full border-black text-2xl">Delete</button>
                  </Link>
                </div>
              </div>);
      ...
      <div className="border-2 p-4 border-black mt-10 shadow-md shadow-black">
        ...
        <div className="flex flex-row flex-wrap gap-5 mt-4">
          {users.map((user) => {
            return (
              <Link to={`/user/${user.id}`} className="flex flex-col justify-between border-2 
        ...
        <Link to={"createUser"} className="flex flex-row mt-5 w-32">
          <button className="bg-gray-400 w-full border-black">Create</button>
        </Link>
      </div>
    </div>);
