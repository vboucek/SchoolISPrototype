...
export const SemesterCreate = () => {
  ...
  const onSubmit: SubmitHandler<ISemesterCreateDto> = (data) => {
    console.log(data);
    axios.post<ISemesterDto, AxiosResponse<ISemesterDto, ISemesterCreateDto>, ISemesterCreateDto>(`semesters`, data)
      .then(response => {
        setCompleted(true);
        reset();
      });
  ...
  return (
    <div className="text-black laptop:mt-20 laptop:mx-20 phone:mt-0 phone:mx-2">
      <h1 className="laptop:text-6xl phone:text-3xl font-bold">Semester creation</h1>
      <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
        ...
        <select className="border-2 border-black" {...register("semesterType", { required: true })}>
          <option key={SemesterType.summer} value={SemesterType.summer}>Summer</option>
          <option key={SemesterType.winter} value={SemesterType.winter}>Winter</option>
        </select>
